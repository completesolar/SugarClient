...
class Sugar_REST {
    ...
    public function create_meeting_attach_to_lead($leadID,
        ...
        $values=array(
                ...
                'customer_meeting_type_c' => $meetingType,
                'appointmentsetter_c' => $meetingSetter,
                'team_name' => 'Global',
                'parent_type' => 'Leads',
                'parent_id' => $leadID,
                'duration_hours'=>2,
    ...
    public function get_relationship($module, $id){
        $call_arguments = array(
                'session'=> $this->session,
                'module_name' => $module,
                'module_ids' => $id,
     ...
     * Description:  Retrieve a list of possible values and its label for a dropdown field.
     * Returns: List of arrays with "name" and "label" indices
     */
    public function get_dropdown_values($module, $fieldName){
        $call_arguments = array(
                'session' => $this->session,
                'module_name' => $module,
                'fields' => array($fieldName)
        );
        $result = $this->rest_request('get_module_fields', $call_arguments);
        $values = array();
        foreach( $result["module_fields"][$fieldName]["options"] as $name=>$arr ){
            $values[] = array("name" =>$arr["name"], "label"=>$arr["value"]);
        }
        return $values;
    }
